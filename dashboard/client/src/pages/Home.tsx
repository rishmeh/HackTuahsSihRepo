...
function SyllabusDialog({ open, onClose, onApprove }: { open: boolean; onClose: () => void; onApprove: (assignments: ExtractedAssignment[]) => void }) {
  const inputRef = useRef<HTMLInputElement | null>(null); const [file, setFile] = useState<File | null>(null); const [assignments, setAssignments] = useState<ExtractedAssignment[]>([]); const [selected, setSelected] = useState<string[]>([]); const [errorMessage, setErrorMessage] = useState(""); const [emptyMessage, setEmptyMessage] = useState("");
  const extractMutation = trpc.syllabus.extract.useMutation({ onSuccess: (result) => { setAssignments(result.assignments); setSelected(result.assignments.map((item) => item.id)); setErrorMessage(""); setEmptyMessage(result.assignments.length ? "" : "No clear assignments were found. Try a syllabus with due dates, homework, projects, or reading lists."); if (!result.assignments.length) toast.info("No clear assignments were found in that file."); }, onError: (error) => { setErrorMessage(error.message || "I couldn't read that syllabus"); setEmptyMessage(""); toast.error(error.message || "I couldn't read that syllabus"); } });
  useEffect(() => { if (!open) { setFile(null); setAssignments([]); setSelected([]); setErrorMessage(""); setEmptyMessage(""); extractMutation.reset(); } }, [open]);
  if (!open) return null;
  const toBase64 = async (source: File) => { const bytes = new Uint8Array(await source.arrayBuffer()); let binary = ""; const chunk = 0x8000; for (let index = 0; index < bytes.length; index += chunk) binary += String.fromCharCode(...Array.from(bytes.subarray(index, Math.min(index + chunk, bytes.length)))); return btoa(binary); };
  const handleExtract = async () => { if (!file) return; setErrorMessage(""); setEmptyMessage(""); try { await extractMutation.mutateAsync({ fileName: file.name, mimeType: file.type || "text/plain", fileBase64: await toBase64(file) }); } catch { /* mutation renders the persistent error state */ } };
  const toggle = (id: string) => setSelected((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id]);
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="syllabus-modal" role="dialog" aria-modal="true" aria-labelledby="syllabus-title"><div className="modal-header"><div><div className="eyebrow eyebrow--coral"><span className="sun-dot" /> Smart planning</div><h2 id="syllabus-title">Turn a syllabus into small steps.</h2><p>Upload a PDF or text syllabus. TableTot will suggest assignments for your review before adding them to today’s rhythm.</p></div><button className="round-more" onClick={onClose} aria-label="Close syllabus upload"><X size={18} /></button></div>{!assignments.length ? <><button className="upload-dropzone" onClick={() => inputRef.current?.click()}><div className="upload-icon"><BookOpen size={22} /></div><strong>{file ? file.name : "Choose a syllabus file"}</strong><span>{file ? "Ready to extract assignments" : "PDF, TXT, MD, or CSV · up to 5 MB"}</span><input ref={inputRef} type="file" accept=".pdf,.txt,.md,.csv,text/plain,text/markdown,text/csv,application/pdf" hidden onChange={(event) => setFile(event.target.files?.[0] ?? null)} /></button>{extractMutation.isPending && <div className="upload-status upload-status--loading">Reading the syllabus and looking for actionable work…</div>}{errorMessage && <div className="upload-status upload-status--error">{errorMessage}</div>}{emptyMessage && <div className="upload-status upload-status--empty">{emptyMessage}</div>}<div className="modal-actions"><button className="modal-secondary" onClick={onClose}>Not now</button><button className="modal-primary" disabled={!file || extractMutation.isPending} onClick={handleExtract}>{extractMutation.isPending ? "Reading syllabus…" : "Find assignments"}<ArrowUpRight size={15} /></button></div></> : <><div className="review-banner"><Sparkles size={16} /><span><strong>{assignments.length} suggestions found.</strong> Keep the ones that belong in your study rhythm.</span></div><div className="assignment-review">{assignments.map((assignment) => <button className={`assignment-review-row ${selected.includes(assignment.id) ? "assignment-review-row--selected" : ""}`} key={assignment.id} onClick={() => toggle(assignment.id)}><span className="review-check">{selected.includes(assignment.id) && <Check size={13} />}</span><span><strong>{assignment.title}</strong><small>{assignment.subject} · {assignment.dueDate} · {Math.round(assignment.confidence * 100)}% confidence</small></span><span className={`priority-pill priority-pill--${assignment.priority}`}>{assignment.priority}</span></button>)}</div><div className="modal-actions"><button className="modal-secondary" onClick={() => { setAssignments([]); setSelected([]); }}>Choose another file</button><button className="modal-primary" disabled={!selected.length} onClick={() => { onApprove(assignments.filter((item) => selected.includes(item.id))); onClose(); }}>{selected.length ? `Add ${selected.length} to tasks` : "Select assignments"}<Check size={15} /></button></div></>}</section></div>;
}
