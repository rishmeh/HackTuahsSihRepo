...
function PomodoroCard({ selectedTask, onStartSession, onCompleteTask, onFocusBlockDone }: { selectedTask?: Task; onStartSession: (taskId: number | null, onRejected: () => void) => void; onCompleteTask: (taskId: number) => void; onFocusBlockDone: () => void }) {
  ...
  // Read the latest seconds through a ref so finishing a block (a side effect)
  // happens once in the interval, not inside a state updater React may replay.
  const secondsRef = useRef(seconds); secondsRef.current = seconds;
  const onFocusBlockDoneRef = useRef(onFocusBlockDone); onFocusBlockDoneRef.current = onFocusBlockDone;
  useEffect(() => { if (!running) return; const timer = window.setInterval(() => { if (secondsRef.current <= 1) { setRunning(false); setSeconds(total); toast.success(mode === "focus" ? "Focus block complete. Time for a small reset." : "Break complete. Ready when you are."); if (mode === "focus") { startedSessionRef.current = false; onFocusBlockDoneRef.current(); } return; } setSeconds((current) => current - 1); }, 1000); return () => window.clearInterval(timer); }, [running, mode, total]);
  const switchMode = (nextMode: "focus" | "break") => { setMode(nextMode); setRunning(false); setSeconds(nextMode === "focus" ? 25 * 60 : 5 * 60); };
  const toggleRunning = () => { const next = !running; setRunning(next); if (next && mode === "focus" && !startedSessionRef.current) { startedSessionRef.current = true; onStartSession(selectedTask?.id ?? null, () => { setRunning(false); startedSessionRef.current = false; }); } };
...
// A few lines per part of the day. The pick changes daily, not on every render.
const GREETINGS: Record<"student" | "parent", Record<DayPart, [string, string][]>> = {
  student: {
    morning: [["Good morning, {name}.", "Fresh mind, fresh start. Let’s make it count."], ["Rise and shine, {name}.", "Your brain is at its sharpest now — let’s use it."], ["Morning, {name}!", "One small block before the day gets busy?"]],
    afternoon: [["Good afternoon, {name}.", "Let’s make a little room for your best thinking."], ["Hey {name}, welcome back.", "Afternoon slump? One focused block beats it."], ["Good afternoon, {name}.", "Halfway through the day — let’s make the next hour good."]],
    evening: [["Good evening, {name}.", "Wind down with one calm, focused block."], ["Evening, {name}.", "A little review now makes tomorrow easier."], ["Hi {name}, good evening.", "Finish strong, then rest well."]],
    night: [["Hey night owl, {name}.", "It’s late — a quick review, then sleep. Your brain files memories while you rest."], ["Still up, {name}?", "Keep it short tonight. Sleep is part of studying too."], ["Good night, {name}.", "Tomorrow’s you will thank tonight’s rest."]],
  },
  parent: {
    morning: [["Good morning, {name}.", "Here’s how {student}’s week is shaping up."]],
    afternoon: [["Good afternoon, {name}.", "A clear picture of {student}’s week, without hovering."]],
    evening: [["Good evening, {name}.", "A clear picture of {student}’s week, without hovering."]],
    night: [["Good evening, {name}.", "Here’s {student}’s day at a glance before you sign off."]],
  },
};

/** Greeting for the current time, re-checked every minute so it turns over at 5pm, 9pm and so on. */
function useGreeting(role: "student" | "parent", name: string, student = "") {
  const [now, setNow] = useState(() => new Date());
  useEffect(() => { const t = window.setInterval(() => setNow(new Date()), 60_000); return () => window.clearInterval(t); }, []);
  const options = GREETINGS[role][dayPart(now.getHours())];
  const [title, subtitle] = options[now.getDate() % options.length];
  const fill = (text: string) => text.replace("{name}", name).replace("{student}", student);
  return { title: fill(title), subtitle: fill(subtitle) };
}
...
function ParentDashboard({ profile }: { profile: ProfileSummary }) {
  ...
  return <div className="page-content parent-page"><div className="page-intro"><div><div className="eyebrow eyebrow--coral"><span className="sun-dot" /> Weekly check-in</div><h1>{greeting.title}</h1><p>{greeting.subtitle}</p></div><div className="parent-intro-actions"><button className="report-button" onClick={exportReport}><Download size={15} /> Download report</button><div className="child-selector"><div className="profile-avatar profile-avatar--small">{studentName.charAt(0).toUpperCase()}</div><span>{studentName}</span></div></div></div><LiveNowCard studentName={studentName} /><div className="metrics-grid parent-metrics"><MetricCard label="Completed" value={kpis ? `${kpis.tasksCompleted} / ${kpis.tasksTotal} tasks` : "—"} note="Real, synced progress" icon={ListChecks} tone="mint" progress={kpis && kpis.tasksTotal ? (kpis.tasksCompleted / kpis.tasksTotal) * 100 : 0} /><MetricCard label="Focus this week" value={progress ? formatMinutes(progress.focusMinutesThisWeek) : "—"} note={kpis ? `${kpis.sessionsCompleted} focus blocks in total` : "Finished focus blocks"} icon={Clock3} tone="coral" /><MetricCard label="Quiz average" value={progress?.quizAveragePercent != null ? `${progress.quizAveragePercent}%` : "—"} note={progress?.quizzesTaken ? `${progress.quizzesTaken} quiz${progress.quizzesTaken === 1 ? "" : "zes"} finished` : "No quizzes finished yet"} icon={Target} tone="sand" /><MetricCard label="Streak" value={kpis ? `${kpis.currentStreakDays} day${kpis.currentStreakDays === 1 ? "" : "s"}` : "—"} note="Days in a row with a focus block" icon={Flame} tone="mint" /></div>{progress && <div className="parent-progress-grid"><FocusWeekCard days={progress.focusByDay} totalMinutes={progress.focusMinutesThisWeek} /><QuizScoresCard quizzes={progress.scoredQuizzes} averagePercent={progress.quizAveragePercent} /></div>}<div className="parent-grid">{progress ? <RecentActivityCard sessions={progress.recentSessions} tasks={progress.recentlyCompletedTasks} /> : <section className="surface-card progress-card"><p className="progress-empty">Loading progress…</p></section>}<ParentalControlsCard studentName={studentName} /></div><section className="privacy-card surface-card privacy-card--wide"><div className="privacy-card-icon"><ShieldCheck size={19} /></div><div><div className="eyebrow"><span className="sun-dot sun-dot--muted" /> Family trust</div><h2>Visibility, not surveillance.</h2><p>Progress is shared as gentle summaries. Notes, voice, and face data stay private on {studentName}'s device.</p></div></section></div>;
}
...
export default function Home({ profile, onLoggedOut }: { profile: ProfileSummary; onLoggedOut: () => void }) {
  ...
  const startSession = (taskId: number | null, onRejected: () => void) => { startFocusMutation.mutate({ taskId }, { onSuccess: (session) => { activeSessionRef.current = { sessionId: session.id, startedAt: Date.now() }; }, onError: (error) => { onRejected(); toast.error(error.message); } }); };
  const finishActiveSession = (maxSeconds?: number) => {
    const session = activeSessionRef.current;
    if (!session) return;
    const elapsed = Math.round((Date.now() - session.startedAt) / 1000);
    completeFocusMutation.mutate({ sessionId: session.sessionId, elapsedSeconds: maxSeconds ? Math.min(elapsed, maxSeconds) : elapsed });
